...
runuser -l $SUDOUSER -c "echo \"$PRIVATEKEY\" > ~/.ssh/id_rsa_key_vault_secret"
...
# issue with SELinux with DOcker and overlays2
openshift_docker_selinux_enabled=false
#openshift_enable_excluders=false
#openshift_enable_docker_excluder=false
openshift_node_groups=[{'name': 'node-config-master', 'labels': ['node-role.kubernetes.io/master=true','region=master','zone=default']}, {'name': 'node-config-infra', 'labels': ['node-role.kubernetes.io/infra=true','region=infra','zone=default']}, {'name': 'node-config-compute', 'labels': ['node-role.kubernetes.io/compute=true','region=app','zone=default']}]
...
sleep 20
runuser -l $SUDOUSER -c "ansible all -b -m service -a \"name=dbus state=restarted\""
runuser -l $SUDOUSER -c "ansible all -b -m service -a \"name=NetworkManager state=restarted\""
runuser -l $SUDOUSER -c "ansible all -b -m service -a \"name=network state=restarted\""
sleep 20
